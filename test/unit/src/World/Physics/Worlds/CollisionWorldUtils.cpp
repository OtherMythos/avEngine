...
TEST(CollisionWorldUtils, shouldObjectsSendEventTestsReturnsTrue){
    ...
    { //Check the type masks.
        ...
        for(int i = 0; i < 7; i++){
            int val0 = AV::CollisionWorldUtils::producePackedInt(AV::CollisionObjectType::CollisionObjectType::RECEIVER, typeMasks[i], AV::CollisionObjectEventMask::LEAVE);
            int val1 = AV::CollisionWorldUtils::producePackedInt(AV::CollisionObjectType::CollisionObjectType::SENDER_SCRIPT, typeMasks[i], AV::CollisionObjectEventMask::LEAVE);
...
TEST(CollisionWorldUtils, shouldObjectsSendEventReturnsFalseWithIncorrectValues){
    ...
    for(int i = 0; i < dataSize; i++){
        int val0 = AV::CollisionWorldUtils::producePackedInt(AV::CollisionObjectType::CollisionObjectType::SENDER_SCRIPT, data[i].first.first, data[i].first.second);
        int val1 = AV::CollisionWorldUtils::producePackedInt(AV::CollisionObjectType::CollisionObjectType::RECEIVER, data[i].second.first, data[i].second.second);
