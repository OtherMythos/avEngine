...
namespace AV{
    ...
    bool DialogCompiler::_parseDialogTag(tinyxml2::XMLElement *item, CompiledDialog& d, BlockContentList* blockList){
        ...
        else if(strcmp(n, tagTypeString(TagType::ACTOR_SET)) == 0){
            AttributeOutput aa;
            GetAttributeResult ar = _getAttribute(item, "a", AttributeType::INT, aa);
            if(ar != GET_SUCCESS){
                mErrorReason = "actorSet tag requires an 'a' attribute for the actor id.";
                return false;
            }
            if(aa.isVariable){
                blockList->push_back({_setVariableFlag(TagType::ACTOR_SET), static_cast<int>(d.vEntry1List->size())});
                VariableAttribute va;
                va._varData = _attributeOutputToChar(aa, AttributeType::INT);
                va.mVarHash = aa.vId;
